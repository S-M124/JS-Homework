...
namespace ProductManagement
{
    ...
    internal class Program
    {
        private static void Main(string[] args)
        {
            ...
            var productTitles = products.Select(p => p.Title).ToList();
            ...
            var highRatedProduct = products.FirstOrDefault(p => p.Rating > 4.8);
            ...
            var descriptionOfFirstRatedProduct = products.FirstOrDefault(p => p.Rating == 4.43)?.Description;

            var lastSkincareUnder50 = products.Where(p => p.Category == ProductCategory.Skincare && p.Price < 50).LastOrDefault()?.Title;

            var firstFragranceAbove100 = products.Where(p => p.Category == ProductCategory.Fragrances && p.Price > 100).FirstOrDefault()?.Title;

            var lastProductLowStockBrand = products.Where(p => p.Stock < 40).LastOrDefault()?.Brand;

            var firstLaptopInStock = products.Where(p => p.Category == ProductCategory.Laptops && p.Stock > 80).FirstOrDefault()?.Title;

            var firstProductAbove1200 = products.FirstOrDefault(p => p.Price > 1200)?.Title;

            var lastSmartphoneStock = products.Where(p => p.Category == ProductCategory.Smartphones).LastOrDefault()?.Stock;

            List<ProductDetails> productDetailsList = products.Select(p => new ProductDetails
            {
                Id = p.Id,
                Title = p.Title,
                Price = p.Price
